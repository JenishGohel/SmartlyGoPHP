...
<aside class="sidebar">
    ...
    <a href="admin-manage-feedback.php" class="bg-white bg-opacity-20">Manage Feedback</a>
    ...
    <div class="overflow-x-auto bg-white rounded-xl shadow-lg">
        <table class="min-w-full text-sm text-left">
            ...
            <?php if ($result && $result->num_rows > 0): ?>
                <?php $i = 1; while ($feedback = $result->fetch_assoc()): ?>
                <tr class="hover:bg-purple-50">
                    <td class="px-4 py-3 font-semibold"><?= $i++; ?></td>
                    <td class="px-4 py-3"><?= htmlspecialchars($feedback['name']); ?></td>
                    <td class="px-4 py-3"><?= htmlspecialchars($feedback['email']); ?></td>
                    <td class="px-4 py-3"><?= htmlspecialchars($feedback['category']); ?></td>
                    <td class="px-4 py-3"><?= htmlspecialchars($feedback['rating']); ?>/5</td>
                    <td class="px-4 py-3">
                        <span class="px-2 py-1 rounded text-white text-xs
                        <?= $feedback['status']=='Resolved' ? 'bg-green-600' : 'bg-yellow-500'; ?>">
                        <?= htmlspecialchars($feedback['status']); ?>
                        </span>
                    </td>
                    <td class="px-4 py-3 max-w-xs truncate" title="<?= htmlspecialchars($feedback['message']); ?>">
